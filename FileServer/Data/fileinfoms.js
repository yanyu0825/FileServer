...
fileinfoms.prototype.newinfo = function (param, action) {
    ...
    mscommand.execute(function (client, cb, sql) {
        var str = 'insert into [file](code,address,createuserid,mimetype) values (@code,@address,@userid,@mimetype)';
...
fileinfoms.prototype.queryinfo = function (param, action) {
    ...
    mscommand.execute(function (client, cb, sql) {
        ...
        client.input('fldName', sql.NVarChar(2000), "[code],[address],[createtime],[createuserid],[mimetype],[forbidden]")//字段
        ...
        client.execute("[DataPagination]", function (err, recordsets, returnValue, affected) {
            if (err)
                ...
                recordsets[0].forEach(function (item) { 
                    data.push({ code: item.code, address: item.address, userid: item.userid, createtime: item.createtime, forbidden: item.forbidden, ext: pathmethod.extname(item.address) })
                })
